...
class Tile {
  ...
  // write a method to draw a polygon that starts at top left corner, go to top right corner, and then makes an arc to bottom left corner
...
// i want to understand how to draw  polygon consisting 
// of left side arc with center 1 at center of the left side of tile arc pointing to the right
//  and the other polygon of left side of tile and arc at the center of the right corner of the tile  pointing to the left
drawLeftSideArc() {
  push();
  translate(this.x, this.y);
  rotate(radians(this.rotation));
  ...
  beginShape();
  vertex(0, 0); // Top left corner
  vertex(0, this.size); // Bottom left corner
  arc(0, this.size / 2, this.size, this.size, HALF_PI, -HALF_PI); // Arc pointing to the right
  endShape(CLOSE);
  pop();
}
...
drawRightSideArc() {
  ...
  beginShape();
  vertex(this.size, 0); // Top right corner
  vertex(this.size, this.size); // Bottom right corner
  arc(this.size, this.size / 2, this.size, this.size, -HALF_PI, HALF_PI); // Arc pointing to the left
  endShape(CLOSE);
  pop();
}
...
// draw quarter pi
  drawArcQUARTER_PI() {
    ...
    beginShape();
    vertex(0, 0); // Top left corner
    arc(0, 0, this.size * 2, this.size * 2, 0, QUARTER_PI); // Arc to bottom left corner
    endShape(CLOSE);
    pop();
  }
  ...
  draw() {
    if (this.type === 'polygon') {
      ...
      this.drawLeftSideArc();
      this.drawRightSideArc();  
    }
  }
}
